...
$fp = fopen(PATH_TO_LMO . '/' . $diroutput . $folder . '/' . basename($file) . '-tab.csv', 'wb');  // TabellenCSV Datei
if ($st > 0) {
    $rounds = $a;
    $act = $st;
} else {
...
for ($i1 = 0; $i1 < $anzsp; $i1++) {
    if (isset($goala[$act - 1][$i1]) && $goala[$act - 1][$i1] == '-1')
        $goala[$act - 1][$i1] = '_';
    if (isset($goalb[$act - 1][$i1]) && $goalb[$act - 1][$i1] == '-1')
        $goalb[$act - 1][$i1] = '_';
}
// $endtab=$act;
$endtab = $anzst;
...
for ($i1 = 0; $i1 < $anzsp; $i1++) {
    if (isset($goala[$act - 1][$i1]) && $goala[$act - 1][$i1] == '_')
        $goala[$act - 1][$i1] = '-1';
    if (isset($goalb[$act - 1][$i1]) && $goalb[$act - 1][$i1] == '_')
        $goalb[$act - 1][$i1] = '-1';
}
$x = 0;
$j = 1;
