...
/** Liga Manager Online 4
  ...
  * 			 	au�er die aktuelle Liga. //nur im Notfall nutzen
  ...
  * Sollte nur genutzt werden wenn Zugriff �ber FTP nicht m�glich ist.
  ...
  *     (die Parameter his_sort und his_template bei Bedarf mit &amp;his_sort=<integer>&amp;his_template=<integer> anh�ngen
  * 
...
// Durch Get bestimmter Parameter (f�r IFRAME)
...
// Direkt bestimmte Parameter (f�r include/require)
...
if (file_exists(PATH_TO_LMO.'/'.$diroutput.$m_liga.'-tab.csv')) {
  ...
  fclose($handle);
  //Umwandeln der �bergebenen Ligen in ein Array
  if($m_ligen != ''){
  ...
  $m_tabelle = $r_tabelle;
  switch($m_sort){
  	case(0): usort($m_tabelle,'cmp');	 break;
  	case(1): usort($m_tabelle,'cmp1');	 break;
  	case(2): usort($m_tabelle,'cmp2');	 break;
  	case(3): usort($m_tabelle,'cmp3');	 break;
	case(4): usort($m_tabelle,'cmp4');	 break;
	case(5): usort($m_tabelle,'cmp5');	 break;
  	case(6): usort($m_tabelle,'cmp6');	 break;
  	case(7): usort($m_tabelle,'cmp7');	 break;
	case(8): usort($m_tabelle,'cmp8');	 break;
  }
  ...
  for ($j=0;$j<$m_anzteams;$j++) {
    ...
    $template->setVariable("TeamBild",HTML_smallTeamIcon($m_liga,$m_tabelle[$j][0]," alt=''"));
